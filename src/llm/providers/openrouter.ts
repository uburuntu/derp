...
import { usdToMicros } from "../../common/money";
import type {
    ChatParams,
    ChatResult,
    ConversationMessage,
...
export class OpenRouterProvider implements LLMProvider {
    ...
    async chat(params: ChatParams): Promise<ChatResult> {
        if (params.tracking?.keyClass !== "paid") {
            throw new Error(
                "OpenRouter fallback is only enabled for paid routes",
            );
        }
        ...
        const callId = params.tracking?.recorder
            ? await params.tracking.recorder.start({
        ...
        const recorder = params.tracking?.recorder;
