...
export class GoogleLLMProvider implements LLMProvider {
    ...
    private async startTracking(
        ...
        if (!tracking?.recorder) return null;
        return await tracking.recorder.start({
    ...
    private async finishTracking(
        recorder: ProviderCallRecorder | undefined,
        callId: string | null,
        model: string,
        usage: TokenUsage,
        input: {
        ...
        if (!recorder || !callId) return costMicros;
        await recorder.finish(callId, {
            actualModelId: model,
    ...
    private async callWithRetries<T>(
        ...
        for (let attempt = 0; attempt < MAX_RETRY_ATTEMPTS; attempt++) {
            ...
            try {
                ...
                if (!canRetry) break;
                await sleep(
                    Math.min(RETRY_DELAY_MS, remainingMs(options.deadlineMs)),
                );
            }
        }
    ...
    async chat(params: ChatParams): Promise<ChatResult> {
        ...
        const attempt = async (): Promise<ChatResult> => {
            ...
            if (params.media && params.media.length > 0) {
                const lastUserIdx = contents.findLastIndex(
                    (c) => c.role === "user",
                );
                const target =
                    lastUserIdx >= 0 ? contents[lastUserIdx] : undefined;
                if (target) {
        ...
        try {
            ...
            const partialCostMicros = partialUsage
                ...
                : undefined;
            await this.failTracking(params.tracking?.recorder, callId, err, {
    ...
    async generateImage(params: ImageParams): Promise<ImageResult> {
        ...
        await this.failTracking(params.tracking?.recorder, callId, error);
        throw error;
    }
                                      ...
                                      referenceImages: [
                                          {
                                              image: {
                                                  imageBytes:
                                                      params.referenceImage.toString(
                                                          "base64",
                                                      ),
                                                  mimeType: "image/jpeg",
                                              },
