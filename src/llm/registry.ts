...
function computeCreditCost(
    ...
    const tokenCost =
        ((inputCostPer1M + outputCostPer1M) * AVG_TOKENS_PER_REQUEST) /
        1_000_000;
...
export function estimateUsageCostUsd(
    ...
    return (
        tokenCost +
        (options.includePerRequest === false ? 0 : model.perRequestCost)
    );
}
...
export function estimateUsageCostMicros(
    ...
    return Math.round(
        estimateUsageCostUsd(modelId, usage, options) * 1_000_000,
    );
}
