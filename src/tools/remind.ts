...
function getReminderMetadata(ctx: ToolContext): {
    ...
    return {
        threadId: getOptionalNumber(metadataCtx, [
            "threadId",
            "messageThreadId",
        ]),
...
async function handleCreate(
    ...
    if (isRecurring) {
        ...
        if (!cronExpression) {
            return {
                text: "cronExpression is required.",
                error: "Missing cron",
            };
        }
...
async function handleCancel(
    ...
    const matches = reminders.filter(
        (r) =>
            ...
            r.id.startsWith(params.reminderId ?? ""),
    );
    if (matches.length > 1) {
        return {
    ...
    if ((reminder.threadId ?? null) !== (ctx.threadId ?? null)) {
        return {
            text: "Reminder not found in this topic.",
            error: "Not found",
        };
    }
...
export const remindTool: ToolDefinition<RemindParams> = {
    ...
    usage: "/remind list; /remind cancel <id>; /remind at <ISO datetime> <message>; /remind cron <cron> | <message>",
