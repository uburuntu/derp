...
async function getParticipantProfilePhoto(
    ...
    const participant = participantRefs.get(
        participantRef.trim().toLowerCase(),
    );
...
chatComposer.on("message", async (ctx) => {
    ...
    if (tier === ModelTier.STANDARD) {
        ...
        if (!chatCreditResult.allowed) {
            const freeModel = getDefaultModel(
    ...
    try {
        ...
        try {
                ...
                async (toolName, args) => {
                    ...
                    if (toolResult.creditResult) {
                        ...
                        if (toolResult.creditResult.creditsRemaining != null) {
                            creditsRemaining =
                                toolResult.creditResult.creditsRemaining;
                        }
                        if (
                            !creditSource &&
            ...
            if (
                ...
                logger.warn(
                    "chat_primary_provider_failed_fallback_openrouter",
                    {
                        error:
                ...
                if ((result.costMicros ?? 0) > maxFallbackCostMicros) {
                    const costUsd = (
                        (result.costMicros ?? 0) / 1_000_000
                    ).toFixed(4);
                    const capUsd = (maxFallbackCostMicros / 1_000_000).toFixed(
                        4,
                    );
                    recordHandledFailure(
                        ...
                        "fallback_cost_over_cap",
                        {
                            model:
                                result.actualModel ??
                                config.openrouterPaidFallbackModel,
        ...
        if (creditsRemaining == null && creditsSpent > 0) {
            ...
            creditsRemaining =
                creditSource === "chat"
                    ? balances.chatCredits
                    : balances.userCredits;
        }
