...
async function runAppliedPaymentSideEffects(
    ...
    try {
        ...
        failures.push(
            `admin: ${err instanceof Error ? err.message : String(err)}`,
        );
    }
    ...
    try {
        ...
        derpMetrics.creditTransactions.add(1, {
            type: effects.transactionType,
        });
    } catch (err) {
        failures.push(
...
function messageThreadId(ctx: DerpContext): number | undefined {
    return (
        ...
        callbackMessage(ctx)?.message_thread_id
    );
}
    ...
    async (ctx) => {
        ...
        if (action === "personal") {
            if (isGroup) {
                await ctx.editMessageText(ctx.t("buy-private-required"), {
                    parse_mode: "HTML",
                    reply_markup: buildBuyTargetKeyboard((key, args) =>
                        ctx.t(key, args),
                    ),
                });
                return;
        ...
        if (action === "group") {
            ...
            await ctx.editMessageText(ctx.t("buy-choose-group"), {
                parse_mode: "HTML",
                reply_markup: buildGroupPackKeyboard((key, args) =>
                    ctx.t(key, args),
                ),
            });
            return;
        ...
        await ctx.editMessageText(ctx.t("buy-choose-subscriptions"), {
            parse_mode: "HTML",
            reply_markup: buildSubscriptionKeyboard((key, args) =>
                ctx.t(key, args),
            ),
        });
    },
...
creditsComposer.callbackQuery(/^sub:(.+)$/, async (ctx) => {
    ...
    try {
        await ctx.api.sendMessage(
            ...
            {
                parse_mode: "HTML",
                message_thread_id: isGroupChat(ctx)
                    ? undefined
                    : messageThreadId(ctx),
                reply_markup: {
                    inline_keyboard: [
...
creditsComposer.callbackQuery(/^pack:(.+)$/, async (ctx) => {
    ...
    try {
        await ctx.api.sendInvoice(
            ...
            [
                {
                    label: ctx.t("buy-invoice-pack-label", {
                        pack: pack.label,
                    }),
                    amount: pack.stars,
                },
...
creditsComposer.callbackQuery(/^group_pack:(.+)$/, async (ctx) => {
    ...
    try {
        await ctx.api.sendInvoice(
            ...
            [
                {
                    label: ctx.t("buy-invoice-group-pack-label", {
                        pack: pack.label,
                    }),
                    amount: pack.stars,
                },
    ...
    if ("errorKey" in validation) {
        await ctx.answerPreCheckoutQuery(
            ...
            ctx.t("payment-validation-error", {
                reason: ctx.t(validation.errorKey),
            }),
        );
        return;
    }
...
creditsComposer.on("message:successful_payment", async (ctx, next) => {
    ...
    if ("errorKey" in validation) {
        await recordInvalidCreditPaymentReceipt(
            ctx,
            payment,
            validation.errorKey,
        );
    ...
    if (payload.type === "sub") {
        ...
        const result = await applyPaymentOrReport(ctx, payment, () =>
            applySubscriptionPayment(
                ...
                },
                {
                    ...
                    isFirstRecurring:
                        subscriptionFields.is_first_recurring === true,
        ...
        const msg =
            result.debtRecovered && result.debtRecovered > 0
                ? ctx.t(
                      isRenewal
                          ? "payment-sub-renewed-debt"
                          : "payment-sub-new-debt",
                      {
                          plan: plan.label,
        ...
        if (payload.target === "chat") {
            ...
            const result = await applyPaymentOrReport(ctx, payment, () =>
                applyChatPackPayment(ctx.db, {
                    ...
                    meta: {
                        packId: pack.id,
                        targetTelegramChatId: payload.targetChatId,
                    },
                }),
            );
            if (!result?.applied) return;
