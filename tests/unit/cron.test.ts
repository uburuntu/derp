...
describe("parseCronToNextDate", () => {
	...
	test("handles step syntax", () => {
		...
		expect(next).not.toBeNull();
		expect((next?.getMinutes() ?? 0) % 15).toBe(0);
	});
...
describe("validateCron", () => {
	...
	test("rejects malformed fields", () => {
		expect(validateCron("not 9 * * *")).toBe("Invalid cron expression");
		expect(validateCron("60 9 * * *")).toBe("Invalid cron expression");
		expect(validateCron("*/0 9 * * *")).toBe("Invalid cron expression");
	});
});
