...
describe("structured chat memory", () => {
	...
	test("adds typed memory items and formats sections", () => {
		...
		raw = addChatMemoryItem(raw, "preference", "Alice wants short answers", {
	...
	test("trims least important oldest items to fit the storage limit", () => {
		...
		expect(trimmed.facts.some((item) => item.text.startsWith("fact 9"))).toBe(
			true,
		);
	});
	...
	test("memory command parses typed prefixes", () => {
		expect(
			memoryTool.parseCommand?.("preference: brief replies", "memory_set"),
		).toMatchObject({
			action: "update",
			kind: "preference",
