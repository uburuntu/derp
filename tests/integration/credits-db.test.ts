...
describe.skipIf(!databaseUrl)("credits database invariants", () => {
    beforeAll(() => {
        ...
        initObservability({
            ...
            openrouterPaidFallbackAllowedModels: ["openai/gpt-5.4-mini"],
    ...
    test("keeps the winning subscription projection and recomputes after refund", async () => {
        ...
        const refund = await reconcileStarRefund(db, laterCharge, {
            test: true,
        });
    ...
    test("reserves free quota atomically under concurrent calls", async () => {
        ...
        expect(results.filter((result) => result === "applied")).toHaveLength(
            3,
        );
        expect(
