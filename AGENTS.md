...
**NEVER manually create or edit generated/codegen files:**
- `migrations/versions/*.py` — Always use `make db-revision MSG="..."` to generate
- `.mo` files — Always use `make i18n-compile` to generate from `.po` files
- Any file marked as "generated" or "autogenerated" in comments

**Always run commands instead of creating files manually** when the command exists. If Docker/database is unavailable, leave a TODO or ask the user to run the command.
