...
async def handle_video(
    ...
    async with db.session() as session:
        ...
        try:
            # Build lightweight deps wrapper for shared function
            ...
            idempotency_key = (
                f"video_generate:{chat_settings.telegram_id}:{message.message_id}"
            )
            ...
            return await message.reply(
                _(
                    "😅 Something went wrong while generating the video. Try again later."
                )
            )
